...
<table class="table responsive table-bordered">
    ...
    $sql = "SELECT *,a.id AS aid FROM consumer_order AS a JOIN consumer_order_detail AS b ON a.id = b.OrderID AND a.OrderOver IS NOT NULL GROUP BY a.OrderNO ORDER BY a.OrderTime DESC";
    //echo $sql;
    $res = mysql_query($sql);
    ...
    $sql = "SELECT *,a.id AS aid FROM consumer_order AS a JOIN consumer_order_detail AS b ON a.id = b.OrderID AND a.OrderOver IS NOT NULL GROUP BY a.OrderNO ORDER BY a.OrderTime DESC LIMIT " . $start . "," . $total;
    ...
    for ($i = 1; $i <= $row; $i++) {
        ...
        <tr align="center">
            ...
            <td width="5%" align="center"><?php echo $ary[$i]['OrderNO']; ?></td>
            <td width="10%">
                <?php
                $addressee = "SELECT * FROM order_address WHERE OrderID='" . $ary[$i]['aid'] . "'";
                $addressee_res = mysql_query($addressee);
                $addressee_row = mysql_fetch_array($addressee_res);
                echo $addressee_row['Recipient'] . "<br>" . $addressee_row['CellPhone'];
                ?>
            </td>
            <!--<td width="3%"><?php echo $ary[$i]['Quantity']; ?></td>-->
            <td width="10%"><?php echo $ary[$i]['OrderTime']; ?></td>
            <td width="4%">
                <?php
                if ($ary[$i]['OrderTime'] != "") {
                    switch ($ary[$i]['OrderStatus']) {
                        case null:
                            echo '訂單無效';
                            break;
                        case 0:
            ...
            <td width="4%">
                <a href="javascript:void(0);" onclick="edit_fun(<?= $ary[$i]['aid'] ?>)" class="btn span1"
